...
int main(int argc, char** argv) {
    ...
    do{
        bool playerWin = false;//initialize both bools to false
        bool enemyWin = false;//only switch to true if they get win condition
    ...
    //cmath multiply card sum to 100
    ...
    EnemySum = vCe1 + vCe2;
      ...
      PlayerSum <= EnemySum?enemyWin=true:enemyWin=false;
    
    PlayerSum >= EnemySum?playerWin=true:playerWin=false;
     
    if(!enemyWin){
        score++; 
        cout<<"Players WIns!"<<endl;
    }
    else if(!playerWin)cout<<"Enemy Wins!"<<endl;
    else cout<<"It is a Tie!" <<endl;
    
    cout << "COngrats! YOu have won "<< score << "time(s)" << endl;
        cout<<"\nAgain:"<<endl; //nested loop
   cin>>option;
   if ((option=='Y')||(option=='y'))
    cout << endl;
